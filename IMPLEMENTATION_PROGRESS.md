...
| **Visual 5-Tab Pinyin Suite & Lesson 0 Experience Overhaul** | Maintenance & Bug Fixes | `Complete` | 100% | Aug 5: Added 9 dynamic rules (Tones, Sandhi, ü-rule, Retroflex, Palatals, Dentals, Tissue Paper Test 💨, Pinyin Typing ⌨️) seeded into Turso DB via `insert_pinyin_rules.js`. Wired `startLesson('hsk1_day0')` & `pinyin-chart-view` into a unified 5-tab suite (`📊 Syllable Matrix`, `👄 Mouth & Airflow Guide`, `🎵 Tone & Typing Rules`, `🎧 Tone Pairs Practice`, `⌨️ Pinyin Typing Game`) with embedded SVG mouth profile graphics and full `ld()` Thai localization. |
