...
| **Thai Translation Patch (HSK1 & HSK2)** | Maintenance & Bug Fixes | `In Progress` | 23% | **Aug 3 update:** Live-verified `hsk1_day1` upgrade to `"llm"` using the updated citation validation fix (`_split_citation()`). 6 lessons now locked on LLM Thai (`hsk1_day1`–`day6`). Attempting `hsk1_day7` hit today's 20 req/day quota ceiling (`429 RESOURCE_EXHAUSTED`); `QuotaExceededError` cleanly stopped the run without corrupting data. 20 HSK1 + 17 HSK2 lessons remain for future quota runs. |
