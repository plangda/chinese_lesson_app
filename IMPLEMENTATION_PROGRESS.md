...
- **In-Progress Tasks**: 0
- **Not Started Tasks**: 2
- **Overall Project Completion**: **~93%** (Core Web App: **~100%**)
...
| **HSK 3 Curriculum Seeding** | Phase 1: Database & Backend | `Complete` | 100% | Remapped to 500 words; all 34 thematic lessons generated and seeded into Turso. |
...
| **Vocab Garden SRS: Active Challenges & Games** | Phase 4: Consistency & Retention | `Complete` | 100% | Implemented stage-based retrieval challenges (Hanzi MCQ, tone ID/input, translation MCQ), interleaved practice, Seed Fusion Lab, and Sentence Quests. (Fixed pinyin input visibility and multi-character tone bugs) |
| **Vocab Garden SRS: API Integration** | Phase 4: Consistency & Retention | `Complete` | 100% | Integrated backend `/api/srs/garden` to return live plant lists and `/api/srs/fuse` to process and save compound fusions. (Fixed fusion pool generation bug & optimized historical sync with bulk SQL query to eliminate N+1 latency) |
...
| **Authentication & Security Hardening (Phase 2)** | Security & Architecture | `Complete` | 100% | Implemented case-insensitive username & optional email support (`COLLATE NOCASE`), transactional registrations, 100% free self-service secret question password recovery with 5-minute JWT reset tokens, rate limiting, global 401 interception (`fetchWithAuth`), and strict `req.user.id` endpoint identity enforcement. |
...
gantt
    ...
    Security & Auth Hardening :done, p10, 2026-08-14, 2026-08-14
    Voice Assessment :milestone, p11, 2026-08-15, 0d
```
...
1. **Phase 1: Database & Backend** (SQLite, REST API, Seeding)
   - **Progress**: 90%
   - *Next Action:* Seeding HSK 4 and higher levels. HSK 1, 2, and 3 translation rollouts are 100% complete and locked on LLM Thai.
...
5. **Phase 5: Settings & UX Polishing** (Settings, Reset, Spring Animations, Responsive Grid)
   - **Progress**: 100%
   - *Next Action*: Complete.
6. **Phase 6: Future Enhancements** (Voice Assessment, Mock Exams, Security Hardening, DevOps)
   - **Progress**: 85%
   - *Next Action*: Plan speech-to-text API integration. Official Mock Exams, Auth v2, and Security Hardening are 100% complete.
