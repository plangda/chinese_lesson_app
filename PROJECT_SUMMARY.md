...
**Overall Completion Status: ~85%** (Core Application is ~95% Complete)
...
- ⏳ **Thai Translation Patch for HSK1 & HSK2:** Run `patch_thai_translations.py` in batches to replace poor Google-Translate-generated Thai with LLM contextual translations via targeted Turso `UPDATE` SQL (no re-seeding needed).
  - Command: `python patch_thai_translations.py --level hsk1 --limit 5` (then repeat for hsk2)
