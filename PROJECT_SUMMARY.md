...
- **Auto-Language Detection in Translation Pipeline:** Configured the post-generation translator (`translate_en_to_th`) to use `sl=auto` rather than hardcoding `sl=en`. This ensures that even if titles or other elements contain Chinese characters, they are auto-detected and translated properly into Thai instead of failing or being skipped.
- **Unique Character Constraint Handling (Polyphonic Words):** When encountering polyphonic words (same character with different pinyins/meanings like `好` as `hǎo` and `hào`) in the same day's "Additional Vocabulary" catch-all pool, they are programmatically distributed to different days (e.g. Day 25 and Day 26) to prevent database unique constraint conflicts.
- **Bilingual Title Prevention:** Added prompt constraints to the Gemini generator templates to enforce English-only titles and prohibit bilingual prefixes (e.g. `我的日常：`) in the generator output, letting the frontend handle structural formatting.
